  ...
  const ProductDetails = () => {
    ...
    return (
      <div className="p-5 max-w-md mx-auto">
          ...
          {selectedTab === "overview" ? (
            <>
              <div className="flex justify-center mt-5">
                <img src={product.img} alt={product.name} className="w-52 h-52 object-contain" />
              </div>

              <div className="mt-5">
                <h2 className="text-lg font-semibold">Reviews ({product.reviews.length})</h2>
                {product.reviews.map((review) => (
                  <div key={review.userId} className="border-b py-3 flex gap-3">
        ...
        <button onClick={handleAddToCart} className="w-full bg-green-500 text-white py-3 rounded-lg mt-5 font-semibold">
          Add To Cart
        </button>
      </div>
    );
